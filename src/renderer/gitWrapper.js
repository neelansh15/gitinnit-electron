...
const config = async (username, email) => {
  const path = globalConfig.getData().current_project.path;
  const git = simpleGit(path);
  const addUsername = ["config", "--global", "user.name", username];
  const addEmail = ["config", "--global", "user.email", email];
  await git.raw(addUsername, (err, result) => {
    console.log(result);
  });
  await git.raw(addEmail, (err, result) => {
    console.log(result);
  });
};
