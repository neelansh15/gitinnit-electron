    ...
    if (this.access_token == "" || this.access_token == null) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const appDataGlobalConfigPath = app.getPath("appData") + "\\" + pkg.name + "\\globalConfig.json";
            console.log(appDataGlobalConfigPath);
            
            //Get the already existing globalConfig.json and update user data to it
            const globalConfigData = JSON.parse(fs.readFileSync(appDataGlobalConfigPath))
            globalConfigData.user = res.data

            fs.writeFileSync(appDataGlobalConfigPath, JSON.stringify(globalConfigData), () => {
